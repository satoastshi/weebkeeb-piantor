            ...
            try:
                ...
                if updated_task:
                    with edit_state() as s:
                        ...
                        if updated_task.status != STATE.current_task.status: s.current_task.status = updated_task.status
                        if updated_task.updated != STATE.current_task.updated: s.current_task.updated = updated_task.updated
                        if updated_task.branch != STATE.current_task.branch: s.current_task.branch = updated_task.branch
                        if updated_task.submodules != STATE.current_task.submodules: s.current_task.submodules = updated_task.submodules
                        # Update other relevant fields as needed
                        if updated_task.started != STATE.current_task.started: s.current_task.started = updated_task.started
                        if updated_task.dependencies != STATE.current_task.dependencies: s.current_task.dependencies = updated_task.dependencies
                        STATE = s
            except (FileNotFoundError, StateError): pass
#!<
...
if STATE.api.todos_clear and tool_name == "Bash":
    ...
    if "python -m sessions.api todos clear" not in command:
        ...
        with edit_state() as s: s.api.todos_clear = False; STATE = s
